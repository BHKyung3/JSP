...
// 	이 3가지는 반드시 기재해야하는 항목으로 처음부터 기재해둬야 함
	...
	// String url = "jdbc:oracle:thin:@localhost:49161:xe"; //접속 url은 지정되어 있음, 변경되지 않음 > oracle 접속
	String url = "jdbc:mysql://localhost:3306/shopdb"; // mysql 접속
	// String uid = "scott"; // 오라클에 있는 정보
	String uid = "testuser";// mysql 있는 정보
	String pass = "1234"; // 오라클에 있는 정보
	String sql = "select * from member"; // 오라클에 있는 정보
%>
<!DOCTYPE html>
<html>
<head>
	...
	<table width = "800" border = "1">
		...
		<%
			try { // try => 예외가 발생할 수 있는 위험한 코드를 감싼다
				// 1. 드라이브 로드(객체생성)
				Class.forName("com.mysql.cj.jdbc.Driver"); // 요거 미기재 시 동작하지 않음
				// 2. DB 연결
